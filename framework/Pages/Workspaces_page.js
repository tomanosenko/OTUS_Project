export async function WorkspacesPage({ page }) {
  ...
  const clickDelete = async () => {
    await page.locator('(//span[contains(@class, "mat-mdc-menu-item-text")])[5]').click();
  };

  const fillDelete = async () => {
    await page.locator('//input[contains(@placeholder, "Please confirm by typing")]').fill('delete');
  };

  const ClickDelete = async () => {
    await page.locator('(//span[contains(@class, "button-touch-target")])[5]').click();
  };
